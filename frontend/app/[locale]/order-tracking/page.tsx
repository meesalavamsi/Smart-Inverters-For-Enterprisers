    ...
    <div className="min-h-screen bg-gray-50 pt-20">
      ...
      <div className="mx-auto max-w-2xl px-4 py-10">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
              />
            </div>
            <button onClick={trackOrder} disabled={loading}
              className="flex items-center gap-2 bg-green-600 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-green-700 disabled:opacity-60 transition-colors">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xl font-extrabold text-green-700">{order.orderNumber}</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-extrabold text-green-700">{formatCurrency(order.totalAmount)}</p>
                  </div>
                </div>
              </div>
                        ...
                        return (
                          <div key={step.key} className="flex items-center gap-4 relative">
                            ...
                            <div>
                              ...
                              {current && <p className="text-xs text-green-600 mt-0.5">Current status</p>}
                            </div>
                          </div>
                        );
                      })}
                ...
                <div className="space-y-3">
                  {order.items.map((item: any) => (
                    <div key={item.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                      ...
                      <p className="font-bold text-green-700 text-sm">{formatCurrency(item.price * item.quantity)}</p>
                    </div>
                  ))}
                </div>
                <div className="flex justify-between pt-3 mt-2 border-t border-gray-100">
                  <p className="font-bold text-gray-900">Total</p>
                  <p className="font-extrabold text-green-700 text-lg">{formatCurrency(order.totalAmount)}</p>
                </div>
                <Link href={`/invoice/${order.orderNumber}`}
                  className="block text-center mt-4 text-green-600 font-semibold text-sm hover:underline">
        ...
        <div className="mt-6 bg-green-50 rounded-2xl p-5 text-center">
          <p className="text-sm text-green-700 font-medium">Need help with your order?</p>
          <div className="flex justify-center gap-4 mt-3">
            <a href="tel:9133639888" className="text-green-600 font-semibold text-sm hover:underline">📞 9133639888</a>
