...
router.post("/", optionalAuth, async (req, res) => {
  ...
  try {
    ...
    // Email to customer (background — non-blocking)
    if (email) {
      sendEmail({
        to: email,
    ...
    // Email notification to manager (background — non-blocking)
    ...
    if (managerEmail) {
      ...
      }).then(() => logger.info(`Manager notified for booking ${booking.bookingNumber}`))
